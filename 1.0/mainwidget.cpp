...
QPoint MainWidget::Position(QPoint(0,0));

MainWidget::MainWidget(QWidget *parent) :
    QWidget(parent), PADDING(2), buttonHeight(20),buttonWidth(20), clickCount(1),isLeftPressDown(false),
    dir(NONE),  ui(new Ui::MainWidget), windowIniWidth((QApplication::desktop()->availableGeometry().width())*29/48),
...
//90,190,81        0,90,171
    ...
    //connect(bottomWidget, &BottomMainWidget::signalMovePoint, this, &MainWidget::slotSignalMovePoint );
    ...
    //播放列表
    listWidget = new PlayListWidget(this);
    ...
    connect(hideButton, &QPushButton::clicked, this, &MainWidget::slotHideButton);
    ...
    stopButton->setStyleSheet("QPushButton{image:url(:/new/player/stop.png); border-radius:20px; background-color: rgb(40,40,40)}"
                                                "QPushButton::hover{background-color: rgb(90,90,90);  image:url(:/new/player/stopHovered.png);}");
   ...
   // QString name = QDir::currentPath();
    ...
    lastVideoButton->setStyleSheet("QPushButton{image:url(:/new/player/backward.png);border-radius:20px; background-color: rgb(40,40,40)}"
                                   "QPushButton:hover{background-color: rgb(90,90,90); image:url(:/new/player/backwardHovered.png)}");
    ...
    playButton->setStyleSheet("QPushButton{image:url(:/new/player/play.png);  border-radius:25px; background-color: rgb(40,40,40)}"
                                                "QPushButton::hover{background-color: rgb(90,90,90); image:url(:/new/player/playHovered.png)}");
    ...
    nextVideoButton->setStyleSheet("QPushButton{image:url(:/new/player/forward.png);border-radius:20px; background-color: rgb(40,40,40)}"
                                                         "QPushButton::hover{background-color: rgb(90,90,90);  image:url(:/new/player/forwardHovered.png)}");
    connect(nextVideoButton, SIGNAL(clicked(bool)), this, SLOT(slotNextVideo()));

    //滑动条
    slider = new ProgressSlider(Qt::Horizontal, bottomWidget);
    slider->setRange(0,0);
    ...
    volumeButton->setStyleSheet("QPushButton{image:url(:/new/player/volume.png);border-radius:15px; background-color: rgb(40,40,40)}"
                                                         "QPushButton::hover{ image:url(:/new/player/volumeHovered.png);border-color:1px black; background-color:rgb(90,90,90);}");
    ...
    playModeListWidget->setStyleSheet("QListWidget{border:none;}"
                                                               "QPlayListWidget::item{background-color:white; font-size:25px;"
   ...
   // totalTimeLabel->setStyleSheet("QLabel{background-color:rgb(190,190,80);}");
    ...
    connect(listWidget, &PlayListWidget::signalClearAll, this, &MainWidget::slotListWidgetClearAll);
    connect(listWidget, &PlayListWidget::signalActionOpen, this, &MainWidget::slotListWidgetActionOpen);
...
//数字显示转换为时间显示
QString MainWidget::digitalToTime(const int& number)
{
    QString timeString;
...
//从路径中得到文件名字
QString MainWidget::getNameFromPath(const QString &fileName)
{
    QString name;
    QChar c;
...
//过滤序号
QString MainWidget::removeIndexFromAppend( QString name)
{
...
void MainWidget::enterEvent(QMouseEvent *event)
{
    qDebug()<<"enter";
}

void MainWidget::leaveEvent(QMouseEvent * event)
{
    qDebug()<<"leave";
...
//设置隐藏按钮
void MainWidget::slotHideButton()
{
...
//鼠标进入目标区域
void MainWidget::slotInBottomRegion()
{
    //qDebug()<<"in";
    bottomWidget->show();
    inBottomRegion = true;
...
void MainWidget::slotInRightRegion()
{
    hideButton->show();
    inRightRegion = true;
}
...
//使用videoWidget0移动窗体
void MainWidget::slotSignalMovePoint(QPoint point)
{
...
//鼠标出去目标区域
void MainWidget::slotOutBottomRegion()
{
    //qDebug()<<"out";
...
void MainWidget::slotOutRightRegion()
{
    //qDebug()<<"out";
    inRightRegion = false;
...
//设置是否隐藏bottomWidget
void MainWidget::slotOutBottomRegionTimer()
{
    if(!inBottomRegion)
    {
        bottomWidget->hide();
...
//设置是否隐藏hideButton
void MainWidget::slotOutRightRegionTimer()
{
    if(!inRightRegion)
    {
        hideButton->hide();
...
//
void MainWidget::slotListWidgetActionOpen()
{
    slotOpenFile();
}
...
//删除列表项
void MainWidget::slotListWidgetItemDeleted(int n)
{
    vector.removeAt(n);
    updateToDataBase();
...
//清空失效记录
void MainWidget::slotListWidgetClearValidItem()
{
...
void MainWidget::slotListWidgetOpenDirectory(int n)
{
...
void MainWidget::slotPlayModeListWidget()
{
    playModeListWidget->hide();
    ...
    switch(row)
    {
        case 0:
        {
            ...
            playModeButton->setStyleSheet("QPushButton{border-radius:15px; image:url(:/new/player/itemOnce.png)}"
        ...
        case 1:
        {
            ...
            playModeButton->setStyleSheet("QPushButton{border-radius:15px; image:url(:/new/player/itemLoop.png)}"
                                          "QPushButton:hover{background-color:rgb(90,90,90); }");
            break;
        }
        case 2:
        {
            ...
            playModeButton->setStyleSheet("QPushButton{border-radius:15px; image:url(:/new/player/sequential.png)}"
                                          "QPushButton:hover{background-color:rgb(90,90,90); }");
            break;
        }
        ...
        case 3:
        {
            ...
            playingMode = Loop;
            playModeButton->setStyleSheet("QPushButton{border-radius:15px; image:url(:/new/player/listLoop.png)}"
                                          "QPushButton:hover{background-color:rgb(90,90,90); }");
            break;
        }
        case 4:
        {
            ...
            playingMode = Random;
            playModeButton->setStyleSheet("QPushButton{border-radius:15px; image:url(:/new/player/random.png)}"
                                          "QPushButton:hover{background-color:rgb(90,90,90); }");
            break;
        }
        default:
    ...
    switch(state)
    {
        case LPlayer::PlayingState:
        {
            ...
            playButton->setStyleSheet("QPushButton{image:url(:/new/player/pause.png);border-radius:25px; background-color: rgb(40,40,40)}"
                                                        "QPushButton::hover{background-color: rgb(90,90,90);image:url(:/new/player/pauseHovered.png)}");
            break;
        }
        default:
        {
...
//
void MainWidget::slotVideoWidgetDropedList(QList<QUrl> urls)
{
    QString fileName;
...
void MainWidget::slotVideoWidgetMouseLeftDown(bool b)
{
...
//设置右上角的三个按钮
void MainWidget::setThreeButton()
{
    //设置三个按钮
...
//    maximumButton->setIcon(maximumIcon);
    closeButton->setStyleSheet("QPushButton{image:url(:/new/player/close.png); border-radius:8px; background-color:rgb(60,65,68);}"
                                                  "QPushButton:hover{background-color:rgb(150,150,150);}");
    minimumButton->setStyleSheet("QPushButton{image:url(:/new/player/minimize.png); border-radius:8px; background-color:rgb(60,65,68);}"
                                                         "QPushButton:hover{image:url(:/new/player/minimizeHovered.png); background-color:rgb(120,120,120);}");
    maximumButton->setStyleSheet("QPushButton{image:url(:/new/player/maximize.png); border-radius:8px; background-color:rgb(60,65,68);}"
                                                        "QPushButton:hover{image:url(:/new/player/maximizeHovered.png); background-color:rgb(120,120,120);}");
...
//响应volumeButton的计时器
void MainWidget::slotVolumeTimer()
{
...
void MainWidget::slotPlayModeTimer()
{
...
void MainWidget::slotPositionChanged(qint64 value)
{
...
//设置当前视频播放的总时间
void MainWidget::slotDurationChanged(qint64 value)
{
...
//使用vector更新database
void MainWidget::updateToDataBase()
{
    //qDebug()<<"updatetoBase()";
    QSqlTableModel mediaTableModel;
    mediaTableModel.setTable("mediatable");
